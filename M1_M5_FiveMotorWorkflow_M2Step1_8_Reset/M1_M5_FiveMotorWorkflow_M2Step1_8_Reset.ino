...
const long M2_STEP = -ONE_TURN / 2;
const long M2_LAYER_TOTAL = ONE_TURN * 2;
...
const int M2_HOMING_BOTTOM_CURRENT = 20;
const int M2_HOMING_BOTTOM_CURRENT_RISE = 6;
...
void setup() {
  ...
  Serial.println();
  Serial.println("========== M1-M5 M2 1/8 Step Reset Workflow Ready ==========");
  Serial.println("One cycle: M2 up 1/2 turn -> M3 approach -> M4 CCW/CW -> M3 small retract -> M5 820/700/820 -> M3 home.");
  Serial.println("After M2 accumulates 2 turns: M1 step -> M2 home -> repeat.");
    ...
    case SYS_M1_STEP:
      ...
      if (m1.done) {
        ...
        if (absLong(m1Progress) >= absLong(M1_TOTAL)) {
          ...
          enterM2HomingDown();
          Serial.println("M1 step done. M2 homing before next 2-turn layer.");
        }
      }
      break;
...
bool checkM2HomingBottomDetected() {
  ...
  bool bottomRiseSignal = false;
  bool bottomAbsoluteSignal = false;
  bool bottomSignal = false;
  bool stoppedAtBottom = absFloat(m2.velocity) < M2_HOMING_BOTTOM_VELOCITY_LIMIT;

  if (current != -1 && currentRise >= M2_HOMING_BOTTOM_CURRENT_RISE) {
    bottomRiseSignal = true;
  }
  ...
  if (current != -1 && abs(current) >= M2_HOMING_BOTTOM_CURRENT) {
    bottomAbsoluteSignal = true;
  }

  if (homingTravel < M2_HOMING_MIN_TRAVEL) {
    bottomSignal = bottomRiseSignal;
  ...
  m2HomingNoMoveStartTime = 0;
  bottomSignal = bottomRiseSignal || bottomAbsoluteSignal;
